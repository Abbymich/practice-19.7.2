...
def test_post_add_pets_with_valid_data(name='Snoop', animal_type='Dog', age='0', pet_photo='images/Snoop 01.jpg'):
...
def test_delete_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.post_add_new_pets(api_key, 'Snoop', 'Dog', '0', 'images/Snoop 01.jpg')
        _, my_pets = pf.get_list_of_pets(api_key, 'my_pets')
...
def test_update_pet_info(name='Снуп', animal_type='измененный', age='1'):
...
def test_add_pets_with_valid_data_without_photo(name='СнупБезФото', animal_type='Русский той', age='2'):
    '''Проверяем возможность добавления нового питомца без фото'''
    _, api_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.post_add_new_pet_without_photo(api_key, name, animal_type, age)

    assert status == 200
    assert result['name'] == name


def test_add_photo_at_pet(pet_photo='images/toy.jpg'):
    ...
    if len(my_pets['pets']) > 0:
        status, result = pf.post_add_photo_of_pet(api_key, my_pets['pets'][0]['id'], pet_photo)
...
def test_add_pet_negative_age_number(name='Мордастик', animal_type='неизвестно', age='-3', pet_photo='images/Мордастик.jpg'):
    ...
    _, api_key = pf.get_api_key(valid_email, valid_password)
    _, result = pf.post_add_new_pet(api_key, name, animal_type, age, pet_photo)

    assert age not in result['age'], 'Питомец добавлен на сайт с отрицательным числом в поле возраст'


def test_add_pet_with_four_digit_age_number(name='Sam', animal_type='cat', age='1234', pet_photo='images/Sam.jpg'):
...
def test_add_pet_with_empty_value_in_variable_name(name='', animal_type='cat', age='2', pet_photo='images/Sam.jpg'):
...
def test_add_pet_with_a_lot_of_words_in_variable_name(animal_type='кот', age='2', pet_photo='images/Max.jpg'):
    ...
    name = 'Дон Диего-д'Эспиноса-де-Хесус-де-ла-Сильва-де- Ибаньец-и-Вальдес'
...
def test_add_pet_with_special_characters_in_variable_animal_type(name='Sam', age='2', pet_photo='images/Sam.jpg'):
